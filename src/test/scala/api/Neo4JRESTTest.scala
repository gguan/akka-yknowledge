...
import graphdb.neo4j.Neo4JRESTClient
...
class Neo4JRESTTest extends FunSuite with BeforeAndAfter {

  implicit lazy val conf: Config = ConfigFactory.load()

  var client = Neo4JRESTClient()
  ...
  test("Neo4JRESTClient node search") {

    // Check DB is empty
    //    val r = client.upsertNode(KGNode(key = "key_1", properties = Map(PROP_TYPE -> List("type_1"), "test" -> "test_1")))
    println(client.findNodeByKey("primary_key_1"))
    println(client.findNodeById(5L))
